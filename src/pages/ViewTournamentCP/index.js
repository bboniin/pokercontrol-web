...
import criciuma from "../../assets/criciuma2.mp4";
...
const videos = [criciuma, criciuma, criciuma];

const ViewTournamentCP = ({ index }) => {
            ...
            <div>
              ...
              <span>
                ...
                /
                {
                  tournament.clients.filter(
                    (item) =>
                      item.exit || (!item.exit && item.chair_tournament),
                  ).length
                }
              </span>
            </div>
                ...
                <span>
                  {
                    tournament.clients.filter(
                      (data) =>
                        (data.chair_tournament || data.exit) && data.timechip,
                    ).length
                  }{" "}
                  TIMECHIP{" | "}
...
export default ViewTournamentCP;
